...
cleanup() {
  ...
  sudo rm -f /tmp/conv0.img /tmp/conv1.img "$STATE_YAML" /tmp/apply1.log /tmp/race1.log /tmp/race2.log
}
trap cleanup EXIT
...
sudo ./dplaned-ci -apply -db-dsn "$DATABASE_DSN" -gitops-state "$STATE_YAML" > /tmp/apply1.log 2>&1 || fail "Initial apply failed"
...
START=$(date +%s%3N)
set +e
APPLY_OUT=$(sudo ./dplaned-ci -apply -db-dsn "$DATABASE_DSN" -gitops-state "$STATE_YAML" 2>&1)
STATUS=$?
set -e
END=$(date +%s%3N)
DURATION=$((END-START))
...
APPLY_FAIL=$(sudo -E ./dplaned-ci -apply -db-dsn "$DATABASE_DSN" -gitops-state "$STATE_YAML" 2>&1 || true)
...
sudo ./dplaned-ci -apply -db-dsn "$DATABASE_DSN" -gitops-state "$STATE_YAML" > /dev/null
...
sudo ./dplaned-ci -apply -db-dsn "$DATABASE_DSN" -gitops-state "$STATE_YAML" > /tmp/race1.log 2>&1 &
PID1=$!
sudo ./dplaned-ci -apply -db-dsn "$DATABASE_DSN" -gitops-state "$STATE_YAML" > /tmp/race2.log 2>&1 &
PID2=$!
...
CONV_STATUS=$(sudo ./dplaned-ci -convergence-check -db-dsn "$DATABASE_DSN" -gitops-state "$STATE_YAML" 2>&1)
