...
func (h *GitSyncHandler) SaveConfig(w http.ResponseWriter, r *http.Request) {
	...
	if req.AuthToken != "" && !strings.Contains(req.AuthToken, "****") {
		if _, err := h.db.Exec(`UPDATE git_sync_config SET auth_token=$1 WHERE id=1`, req.AuthToken); err != nil {
			log.Printf("GIT-SYNC: failed to update auth token: %v", err)
		}
	}
	// Clear token if switching away from token auth
	if req.AuthType != "token" {
		if _, err := h.db.Exec(`UPDATE git_sync_config SET auth_token='' WHERE id=1`); err != nil {
			log.Printf("GIT-SYNC: failed to clear auth token: %v", err)
		}
	}
...
func (h *GitSyncHandler) Pull(w http.ResponseWriter, r *http.Request) {
	...
	if syncErr != nil {
		if _, err := h.db.Exec(`UPDATE git_sync_config SET last_error=$1, last_sync_at=$2 WHERE id=1`,
	...
	if _, err := h.db.Exec(`UPDATE git_sync_config SET last_sync_at=$1, last_commit=$2, last_error='' WHERE id=1`,
		time.Now().Format(time.RFC3339), commit); err != nil {
...
func (h *GitSyncHandler) Push(w http.ResponseWriter, r *http.Request) {
	...
	if req.Message == "" {
		req.Message = fmt.Sprintf("Export stack: %s (via DPlaneOS)", req.StackName)
	}
	...
	commit := h.getLastCommit(cfg.LocalPath)
	if _, err := h.db.Exec(`UPDATE git_sync_config SET last_commit=$1, last_sync_at=$2 WHERE id=1`,
		commit, time.Now().Format(time.RFC3339)); err != nil {
		log.Printf("GIT-SYNC: failed to update push status: %v", err)
	}
...
func (h *GitSyncHandler) generateCompose(containers []map[string]interface{}) string {
	...
	for _, c := range containers {
		...
		if networkMode != "host" && !strings.HasPrefix(networkMode, "container:") {
			for netName := range getMap(getMap(c, "NetworkSettings"), "Networks") {
				if netName != "bridge" && netName != "host" && netName != "none" {
					allNetworks[netName] = true
				}
			}
		}
	...
	b.WriteString("# Generated by DPlaneOS Git Sync\n")
	...
	for _, c := range containers {
		...
		// Restart policy - read from actual config, not hardcoded
		restartPolicy := "no"
		if rp := getMap(hostConfig, "RestartPolicy"); len(rp) > 0 {
			switch getString(rp, "Name") {
		...
		// Network mode (host / container:X) or per-network references
		networkMode := getString(hostConfig, "NetworkMode")
		if networkMode == "host" || strings.HasPrefix(networkMode, "container:") {
	...
	// Top-level networks block - compose rejects service-level network refs without this
	if len(allNetworks) > 0 {
		netNames := make([]string, 0, len(allNetworks))
		for n := range allNetworks {
	...
	// Top-level volumes block for named volumes
	if len(allVolumes) > 0 {
		volNames := make([]string, 0, len(allVolumes))
		for v := range allVolumes {
	...
	go func() {
		...
		for range ticker.C {
			...
			if syncErr != nil {
				if _, err := h.db.Exec(`UPDATE git_sync_config SET last_error=$1, last_sync_at=$2 WHERE id=1`,
					syncErr.Error(), time.Now().Format(time.RFC3339)); err != nil {
					log.Printf("GIT-SYNC: failed to save auto-sync error: %v", err)
				}
				...
				if _, err := h.db.Exec(`UPDATE git_sync_config SET last_sync_at=$1, last_commit=$2, last_error='' WHERE id=1`,
					time.Now().Format(time.RFC3339), commit); err != nil {
					log.Printf("GIT-SYNC: failed to update auto-sync status: %v", err)
				}
