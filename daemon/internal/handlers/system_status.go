...
func (h *SystemStatusHandler) HandleProfile(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("WARN: os.Hostname: %v", err)
		hostname = "unknown"
	}
...
func (h *SystemStatusHandler) HandlePreflight(w http.ResponseWriter, r *http.Request) {
	...
	dfOut, err := cmdutil.RunFast("df", "-h", "/")
	diskMsg := "Unable to check"
	if err != nil {
		log.Printf("WARN: df: %v", err)
	} else {
	...
	memOut, err := cmdutil.RunFast("free", "-h")
	memMsg := "Unable to check"
	if err != nil {
		log.Printf("WARN: free: %v", err)
	} else {
		lines := strings.Split(string(memOut), "\n")
		if len(lines) > 1 {
			memMsg = strings.TrimSpace(lines[1])
		}
	}
