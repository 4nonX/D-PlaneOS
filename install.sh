...
# Secure DB file — contains auth tokens
chmod 600 /var/lib/dplaneos/dplaneos.db 2>/dev/null || true
# Create git-stacks directory for Git Sync feature
mkdir -p /var/lib/dplaneos/git-stacks
...
# Install ZFS mount gate service — CRITICAL: prevents Docker-before-ZFS race
if [ -f "$SCRIPT_DIR/systemd/dplaneos-zfs-mount-wait.service" ]; then
    cp "$SCRIPT_DIR/systemd/dplaneos-zfs-mount-wait.service" /etc/systemd/system/
    # Install Docker override drop-in
    mkdir -p /etc/systemd/system/docker.service.d
    cp "$SCRIPT_DIR/systemd/docker.service.d/zfs-dependency.conf" /etc/systemd/system/docker.service.d/
    systemctl daemon-reload
    systemctl enable dplaneos-zfs-mount-wait.service
    log "✓ ZFS mount gate installed (prevents Docker-before-ZFS data loss)"
else
    warn "ZFS mount gate service not found — Docker may start before ZFS pools are ready"
fi
...
# Start services
# Create expected-pools.conf so the ZFS gate knows which pools to wait for
mkdir -p /etc/dplaneos
if command -v zpool >/dev/null 2>&1; then
    CURRENT_POOLS=$(zpool list -H -o name 2>/dev/null || true)
    if [ -n "$CURRENT_POOLS" ]; then
        echo "# D-PlaneOS: ZFS pools that MUST be mounted before Docker/daemon start" > /etc/dplaneos/expected-pools.conf
        echo "# Auto-generated by install.sh — edit to add/remove pools" >> /etc/dplaneos/expected-pools.conf
        echo "$CURRENT_POOLS" >> /etc/dplaneos/expected-pools.conf
        log "✓ ZFS gate pool list: $(echo $CURRENT_POOLS | tr '\n' ' ')"
    else
        log "No pools yet — ZFS gate will auto-detect on first pool creation"
    fi
fi
